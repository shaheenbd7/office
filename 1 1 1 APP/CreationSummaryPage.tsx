import React, { FormEvent, useEffect, createRef } from 'react';
...
export default function CreationSummaryPage({ rootJson }: CreationParams) {
  ...
  function getProfileName() {
    return isFilled(rootJson._profileName);
  }
  ...
  function getPassword() {
    const pass = rootJson._description;
    return pass == '' ? '********' : pass;
  }
  ...
  function NetworkInfoSection() {
    return (
      <div>
        ...
        <h3 className={stylesSP.sectionSubTitle}>
          {SUMMARY_FORM_STRINGS.NETWORK_INFO_FIELD10}
        </h3>
        <h5 className={stylesSP.sectionText}>{getProxy()}</h5>
        <h3 className={stylesSP.sectionSubTitle}>
